...
public class ImportReceiptReport extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (supplierIdRaw != null && !supplierIdRaw.trim().isEmpty()) {
            try { supplierId = Long.parseLong(supplierIdRaw.trim()); } catch (Exception ignored) {}
        }
        ...
        try {
            ...
            int totalPhoneQty = dao.getTotalPhoneQty(from, to, supplierId);
 
            // Product-level aggregation for the table
            int totalItems  = dao.countByProduct(from, to, supplierId);
            int totalPages  = (int) Math.ceil(totalItems * 1.0 / pageSize);
            if (totalPages < 1) totalPages = 1;
            if (page > totalPages) page = totalPages;
 
            List<ProductQuantitySummary> productRows = dao.listByProduct(from, to, supplierId, page, pageSize);
