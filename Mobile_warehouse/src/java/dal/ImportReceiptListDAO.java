...
public class ImportReceiptListDAO {
    ...
    public boolean deleteDraft(long importId) throws SQLException {
        ...
        try (Connection con = openConn()) {
            ...
            if (curStatus == null) { con.rollback(); return false; }
    ...
    // ✅ FIX: countByUiStatus không lọc theo uiStatus (tab counts luôn là tổng)
    public Map<String, Integer> countByUiStatus(String q, LocalDate from, LocalDate to) throws SQLException {
        ...
        String sql = "SELECT ir.status, COUNT(*) AS cnt FROM import_receipts ir WHERE 1=1 "
        ...
        try (Connection con = openConn();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    String uiKey = mapDbToUiStatus(dbStatus);
                    m.put(uiKey, m.getOrDefault(uiKey, 0) + cnt);
                }
            }
            m.put("all", total);
        }
    ...
    public List<ImportReceiptListItem> list(String q, String uiStatus, LocalDate from, LocalDate to,
        ...
        String sql = "SELECT ir.import_id, ir.import_code, s.supplier_name, "
                ...
                + "WHERE 1=1 "
                + buildWhere(q, uiStatus, from, to)
                + "GROUP BY ir.import_id, ir.import_code, s.supplier_name, "
                + "         u.full_name, ir.receipt_date, ir.status "
    ...
    private String buildWhere(String q, String uiStatus, LocalDate from, LocalDate to) {
        ...
        // ✅ FIX: bỏ qua điều kiện status khi là "all" hoặc null/blank
        if (uiStatus != null && !uiStatus.isBlank() && !"all".equalsIgnoreCase(uiStatus)) {
    ...
    private String mapUiStatusToDbCondition(String uiStatus) {
        ...
        switch (uiStatus.toLowerCase()) {
            case "all":       return "1=1";
            case "pending":   return "(ir.status IN ('DRAFT','PENDING'))";
            case "completed": return "(ir.status='CONFIRMED')";
            case "cancelled": return "(ir.status IN ('CANCELED','CANCELLED'))";
            default:          return "1=1";
        }
    }
}